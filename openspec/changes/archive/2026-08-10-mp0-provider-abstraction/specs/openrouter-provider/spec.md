...
#### Scenario: ChatStream returns channel

- **WHEN** `ChatStream(ctx, req)` is called
- **THEN** the provider SHALL return a `<-chan domain.StreamEvent` (streaming SSE parsing is implemented in MP1; for MP0 this method MAY return a not-implemented error or delegate to a stream client)
