...
Defines the `cmd/extract` CLI and `ExtractUseCase` that turn free-form text into structured JSON via `tools.Extract[T]`, with a predefined `ExtractionResult` schema, retry-wrapped provider, and a stdin-to-stdout pipeline.
...
The system SHALL define an `ExtractionResult` struct in `internal/core/usecases` with fields: `Summary string` (json `summary`), `Entities []Entity` (json `entities`), `ActionItems []string` (json `action_items`), `Dates []string` (json `dates`), `Amounts []string` (json `amounts`). The `Entity` struct SHALL have fields `Name string` (json `name`) and `Type string` (json `type`). The tool schema SHALL be built once at startup via a hand-built schema constructor (`buildExtractSchema`) that describes array items explicitly, and reused for every extraction call. The CLI SHALL NOT support custom schemas via flags in Phase 1.

### Scenario: Schema built at startup

- **WHEN** the extraction use case is initialized
- **THEN** it SHALL build the `ToolSchema` via `buildExtractSchema()` to produce a schema with explicit array item types and use that schema for all subsequent `Extract[T]` calls
